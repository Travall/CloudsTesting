...
public class NoiseGPU implements Disposable {
	...
	//private final ShaderProgram viewer;
	...
	public NoiseGPU() {
		...
		final FrameBufferBuilder builder = new FrameBufferBuilder(256, 256);
		...
		//viewer = new ShaderProgram(files.internal("Shaders/viewer.vert"), files.internal("Shaders/viewer.frag"));
	}
	
	float z = -1024f;
	
	public void noise(float offset) {
		...
		shader.bind();
		shader.setUniformi("octave", 5);
		shader.setUniformf("scale", 5f);
		shader.setUniformf("gain", 0.4f);
		shader.setUniformf("z", z);
